...
var BareFoot = (function () {
	...
	function BareFoot() {
		...
		var DEFAULTS = {
			...
			footnotesQuery: "[id^='user-content-fn']",
			supQuery: 'a[href^="#user-content-fnref"]',
			fnButtonMarkup:
	...
	_createClass(BareFoot, [
		...
		},
		{
			...
			value: function calculateOffset(fn, btn) {
				...
				tipOffset =
					contOffset - wrapOffset + contWidth / 2 - tipWidth / 2;
		...
		},
		{
			...
			value: function resizeAction() {
				...
				var footnotes = document.querySelectorAll(
			...
			value: function init() {
				...
				[].forEach.call(this.footnotes, function (fns, i) {
					...
					[].forEach.call(fns, function (fn, i) {
						...
						fnRefN = i + 1;
						fnHrefId = fn
							.querySelector(_this6.config.supQuery)
							.getAttribute('href');

						fnContent = _this6.removeBackLinks(
							fn.innerHTML.trim(),
							fnHrefId
						);
						...
						ref = currentScope.querySelector(
							fnHrefId.replace(':', '\\:')
						);

						footnote =
							'<div class="' +
							_this6.config.fnContainer +
							'">' +
							_this6.buildButton(
								fnHrefId,
								fn.id,
				...
				[].forEach.call(
					...
					function (el) {
						el.addEventListener(
							'click',
							_this6.clickAction.bind(_this6)
						);
					}
				);
